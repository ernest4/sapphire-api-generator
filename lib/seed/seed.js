...
exports.seed = async (asset, options, subPath) => {
  ...
  `);
    ...
    console.log(generatePopulatedModelObject(asset, modelCopy, i));
    console.log(randomString(null, i));
    let preSave = new Asset(generatePopulatedModelObject(asset, modelObject, i));
    preSave.save((err, asset) => {
      if (err) throw new Error(err);
      console.log(`seeded ${i}/${option.count}`);
    });
  }
  return;
};
