...
const { inlineSource, jsonBackedSchemaSource } = require("./fileTemplates/allModels.source");
...
const { modelInlineSource, modelJavaScriptSource } = require("./fileTemplates/model.source");
...
function createServerFileSync(app_name, options) {
  try {
    fs.writeFileSync(`${app_name}/server.js`, serverSource(app_name, options));
...
function createServiceFileSync(asset, options) {
  ...
  try {
    ...
    fs.writeFileSync(servicePath, serviceSource(asset));
