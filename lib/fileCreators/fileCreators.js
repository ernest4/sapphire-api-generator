...
const { serverSourceUpdate } = require("./fileTemplates/js/serverUpdate.source");
...
function appendServerFileSync(asset, options) {
  try {
    const sapphireJSON = fs.readFileSync(`./sapphire.json`);
    const appName = JSON.parse(sapphireJSON).appName;

    // testing functionality, this object will be dynamically created
    const changes = [
      {
        prev: `const dotEnv = require("dotenv");
        const customCors = require("./api/shared/cors");`,
        new: `const dotEnv = require("dotenv");
    ...
    const path = `${appName}/server.js`;
    const source = serverSourceUpdate(appName, options, changes);
    return createFileSync(path, source);
  } catch (err) {
    throw new Error(err);
  }
...
function createControllerFileSync(asset, options) {
  ...
  const source = controllerSource(asset);
  return createFileSync(path, source);
}
