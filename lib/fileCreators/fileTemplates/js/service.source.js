...
// imbedded in request bodies directly.
...
exports.show${capitalize(asset)} = async ${asset}Id => {
  try {
    let ${asset} = await ${capitalize(asset)}.findById(${asset}Id);

    // if you want to have multi document relationship, the commented out lines
    // below are an example of code you need to add for referencing to work.
