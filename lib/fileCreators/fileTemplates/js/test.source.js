...
function generateTestCaseGET(asset, options, model) {
  ...
  const shouldTests = generateShouldLines(saveFieldsObject);
...
function generateShouldLines(saveFieldsObject) {
  console.log(`
  save fields OBJ
  `);
  console.log(saveFieldsObject);

  let finalShouldLinesString = `// currently this isn't meant to be exhaustively deep, rather a quick check that
            // object is mostly what it should be...`;

  for (let field in saveFieldsObject) {
    finalShouldLinesString += `
            ${generateTestLine(field, saveFieldsObject)}`;
  }
...
function generateTestLine(field, saveFieldsObject) {
  ...
  let extraCheck = generateTestLineExtraCheck(field, saveFieldsObject);

  const testLine = `${basicTest}${extraCheck};`;
  return testLine;
}

function generateTestLineExtraCheck(field, saveFieldsObject) {
  if (!saveFieldsObject[field]) return "";

  if (Array.prototype.isPrototypeOf(saveFieldsObject[field]))
    return `.eql([${saveFieldsObject[field]}])`;

  return `.eql("${saveFieldsObject[field]}")`; // must be string
}
