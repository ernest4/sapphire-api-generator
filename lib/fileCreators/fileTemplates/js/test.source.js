...
function generateSaveFields(model) {
  ...
  let saveFieldsString = "";

  saveFieldsObject = generatePopulatedModelObject(model);

  // process the string to fit into the code section
  saveFieldsString = JSON.stringify(saveFieldsObject, null, 2);
  // saveFieldsString = saveFieldsString.replace(/:{"/g, `: { "`);
  // saveFieldsString = saveFieldsString.replace(/\s*}/g, ",\n\t\t\t\t}");

  console.log(saveFieldsString); // TESTING

  return saveFieldsString;
}

function generatePopulatedModelObject(model) {
  // base case
  if (model.type) {
    return assignValueToType(model.type);
  }

  // recursive case
  for (let field in model) {
    if (field === "createdDate" || field === "auth") {
      delete model[field];
...
function assignValueToType(type) {
  // use random seed data to populate string fields...
