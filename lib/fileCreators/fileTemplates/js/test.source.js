...
const { randomName, randomDate, randomNumber, randomString } = require("../../../seed/seed.data");
...
function generateTestCaseGET(asset, options, model) {
  ...
  return `describe("GET /${asset}", function() {
    it("it should get all the ${asset}", function(done) {
      chai
        .request(server)
  ...
  describe("GET /${asset}/:id", () => {
    it("it should GET ${asset} by the given id", done => {
...
function generateTestCasePOST(asset, options, model) {
  // let negativeTest = `it("it should not POST a ${capitalize(asset)} without pages field ${options.auth ? `with Authorization` : ""}", done => {
  //     let ${asset} = {
  ...
  let positiveTest = `it("it should POST a ${asset} ${
    options.auth ? `with Authorization` : ""
  }", done => {
...
function generateTestCasePUT(asset, options, model) {
  const {
    ...
    saveFieldsObject,
    saveFieldsModified,
    saveFieldsObjectModified
  } = generateSaveFields(asset, model);
  const shouldTestsModified = generateShouldLines(saveFieldsObjectModified, 12);

  return `
  describe("PUT /${asset}/:id", () => {
    it("it should UPDATE a ${asset} given the id ${
    options.auth ? `with Authorization` : ""
  }", done => {
      let ${asset} = new ${capitalize(asset)}(${saveFields});
      ${asset}.save((err, ${asset}) => {
...
function generateTestCaseDELETE(asset, options, model) {
  ...
  describe("DELETE /${asset}/:id", () => {
    it("it should DELETE a ${asset} given the id ${
    options.auth ? `with Authorization` : ""
  }", done => {
      let ${asset} = new ${capitalize(asset)}(${saveFields});
      ${asset}.save((err, ${asset}) => {
...
function generateSaveFields(asset, originalModel) {
  ...
  let saveFieldsObjectModified = {};
  let rawSaveFieldsString = "";
  let rawSaveFieldsStringModified = "";
  let saveFields = "";
  let saveFieldsModified = "";
  ...
  // create a slightly modified version for update PUT tests...
  saveFieldsObjectModified = JSON.parse(JSON.stringify(saveFieldsObject));
  saveFieldsObjectModified["auth"] = true;

  // process the string to fit into the code section
  rawSaveFieldsStringModified = JSON.stringify(saveFieldsObjectModified, null, 2);

  // add different indentation levels to different lines
  const linesModified = rawSaveFieldsStringModified.split(/\r?\n/);
  for (let i = 0, line = ""; i < linesModified.length; i++) {
    line = linesModified[i];
    if (i === 0) line = `${line}\n`;
    else if (i === linesModified.length - 1) line = `\t\t\t\t\t${line}`;
    else line = `\t\t\t\t\t${line}\n`;

    saveFieldsModified += line;
  }

  return { saveFields, saveFieldsObject, saveFieldsModified, saveFieldsObjectModified };
}

// function generatesaveFieldsObjectModified(saveFields, saveFieldsObject) {
//   let saveFieldsObjectModified = JSON.parse(JSON.stringify(saveFieldsObject));
//   const name = randomName();
//   saveFieldsObjectModified["newField"] = name;

//   let saveFieldModified = `"newField" : "${name}"`;

//   // add different indentation levels to different lines
//   const lines = saveFields.split(/\r?\n/);
//   for (let i = 0, line = ""; i < lines.length + 1; i++) {
//     line = lines[i];
//     if (i === 0) line = `${line}\n`;
//     else if (i === lines.length - 1) line = `\t\t\t${line}`;
//     else if (i === lines.length) line = `\t\t\t${saveFieldModified}`;
//     else line = `\t\t\t${line}\n`;

//     saveFields += line;
//   }

//   let saveFieldsModified = saveFields;

//   return { saveFieldsModified, saveFieldsObjectModified };
// }
...
function generateTestLine(field, saveFieldsObject) {
  // dates are special because if you feed in raw number, mongoose will automatically
  // cast it to tade object and the comparison will have to happen against a date object
  // printed out as ISO time string
  if (saveFieldsObject[field].date) {
    return `res.body.should.have.property("${field}").eql(new Date('${
      saveFieldsObject[field].date
    }').toISOString())`;
  }
...
function generatePopulatedModelObject(asset, model, seed) {
  // base case
  if (model.type) return assignValueToField(asset, model, null, seed);

  // recursive case
  for (let field in model) {
    if (field === "createdDate" || field === "auth") delete model[field];
    else model[field] = generatePopulatedModelObject(asset, model[field], seed);
  }

  return model;
}

exports.generatePopulatedModelObject = generatePopulatedModelObject;

function assignValueToField(asset, field, fieldof, seed) {
  ...
  switch (type) {
    // basic types
    case "String":
      value = generateString(field, seed);
      break;
    // case "Date":
...
function generateString(field, seed) {
  ...
  if (field.hasOwnProperty("unique")) return randomString(null, seed || Date.now());
