...
const fs = require("fs");
const directoryCreator = require("./dirCreators");
...
exports.load = function(schemaFile) {
  try {
    ...
    for (let field in schemaObject) {
      console.log(schemaObject[field]);
      findAndConvertTypes(schemaObject[field]);
      console.log(schemaObject[field]);
    }
...
function createModelFileSync(asset, options) {
  try {
    ...
    // TODO: check if inline is true and alter the models project structure
    // generation accrodingly
...
let ${capitalize(asset)}Schema = new mongoose.Schema(
  mongooseJSONSchemaLoader.load("./api/v1/models/${asset}/${asset}.schema.json"),
  {
    getters: true
  }
);
