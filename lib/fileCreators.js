...
function createReadMeFileSync(app_name, options) {
  try {
    ...
    const readmeSource = 
`
# This project was generated with [Sapphire](https://github.com/ernest4/sapphire-api-generator)
...
function createIntroTXTFilesSync(app_name, options) {
  try {
    if (options.intro) {
      ...
      const utilsIntroSource = 
`
This folder is for utterly generic utility functions and classes which are not specific to web apps
...
`;
      const sharedIntroSource = 
`
Code that is broadly shared all throughout the web app shall go here. Even if the code is a once
off used in the app, if it doesn't really belong in services, models, controller etc. it shall go
here.
`;
      const controllerIntroSource = 
`
Controllers extract the data from requests pass it services and return their responses to users.
Controllers get called by the routes handler.

Controllers are 'thin' and minimal.
`;
      const modelsIntroSource = 
`
Models encapsulate the database information such as schemas, validations, indexes etc.
...
`;
      const routesIntroSource = 
`
Routes tie together the URLs and controllers and any acompanying middleware such as authorization.
...
`;
      const servicesIntroSource = 
`
Services is where all the business log lives. Services are 'fat' and contain everything they need
...
array.push(createIntroTXTFilesSync);
array.push(createMiddlewareFilesSync);
